...
def generate_temperature_data(n_years=10, n_stations=10, n_ensemble_members=10):
    ...
    for i in range(n_stations):
        ...
        modeled_temps[:, i] = (
            base_temp + seasonal_cycle + station_bias +
            model_bias + correlation * noise + (1 - correlation) * model_noise
        )
...
def generate_wind_data(n_years=5, n_stations=5):
    ...
    for i in range(n_stations):
        ...
        observed_wind_speed[:, i] = (
            base_speed + diurnal_cycle + seasonal_cycle +
        ...
        modeled_wind_speed[:, i] = (
            base_speed + diurnal_cycle + seasonal_cycle +
            speed_bias + correlation * speed_noise * np.random.uniform(0.5, 1.5) +
...
def generate_spatial_data():
    ...
    for t in range(n_days):
        ...
        modeled_field[t] = (
            large_scale + model_bias +
            correlation * correlated_noise + (1 - correlation) * model_noise
        )
