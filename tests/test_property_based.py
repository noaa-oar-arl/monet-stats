...
from src.monet_stats.contingency_metrics import POD, FAR, CSI, HSS, ETS, TSS
from src.monet_stats.correlation_metrics import R2, RMSE, IOA, KGE, pearsonr
from src.monet_stats.error_metrics import MAE, MB, NRMSE
from src.monet_stats.relative_metrics import NMB, NME
from src.monet_stats.efficiency_metrics import NSE, MSE, MAPE
from src.monet_stats.utils_stats import circlebias, angular_difference
...
class TestPropertyBased:
    ...
    @given(angle1=st.floats(min_value=0, max_value=360, allow_nan=False, allow_infinity=False),
    ...
    @given(
        arr1=arrays(dtype=np.float64, shape=st.integers(min_value=5, max_value=20), 
                    elements=st.floats(min_value=0.1, max_value=100, allow_nan=False, allow_infinity=False)),
        arr2=arrays(dtype=np.float64, shape=st.integers(min_value=5, max_value=20), 
                    elements=st.floats(min_value=0.1, max_value=100, allow_nan=False, allow_infinity=False))
    )
    @settings(suppress_health_check=[HealthCheck.too_slow], deadline=1000)
