...
class TestEfficiencyMetrics:
    ...
    def test_tolerance_in_pc_metric(self):
        ...
        assert pc_result >= expected_pc, f"PC calculation inconsistent"


class TestEfficiencyMetricsHypothesis:
    """Property-based tests using Hypothesis."""

    @given(arrays(np.float64, 10, elements=st.floats(min_value=-1000, max_value=1000, allow_nan=False, allow_infinity=False)))
    ...
    @given(
        arrays(np.float64, 10, elements=st.floats(min_value=1, max_value=100, allow_nan=False, allow_infinity=False)),
        arrays(np.float64, 10, elements=st.floats(min_value=1, max_value=100, allow_nan=False, allow_infinity=False))
    )
    ...
    @given(
        arrays(np.float64, 5, elements=st.floats(min_value=-100, max_value=100, allow_nan=False, allow_infinity=False)),
        arrays(np.float64, 5, elements=st.floats(min_value=-100, max_value=100, allow_nan=False, allow_infinity=False))
    )
